# fakenewsnet Visualization Report
...
**Trend seen:** `real` dominates the label mix at 72.2%, well above `fake` by 40.0.
...
**Trend seen:** All observed modality values fell into `text`.
...
**Trend seen:** `without context` dominates the context coverage mix at 100.0%, well above `with context` by 90.0.
...
**Trend seen:** `recall` is the strongest metric at 1.000, while `precision` is the main constraint at 0.278.
